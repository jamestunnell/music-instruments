...
class InstrumentKey
  ...
  ARG_SPECS = {
    ...
    :inactivity_threshold => arg_spec(:reqd => false, :type => Float, :default => 1.0e-5),
    :inactivity_timeout_sec => arg_spec(:reqd => true, :type => Float),
    :pitch_range => arg_spec(:reqd => true, :type => Range, :validator => ->(a){ a.max > a.min }),
    :start_pitch => arg_spec(:reqd => true),
